...
void OutputCreator::tetrahedraToSeparateTriangles(std::vector<PointD3>& points, std::vector<PointD3>& tris, int lastCam) {
	std::vector<Delaunay3::Vertex_handle> vecBoundsHandles;
	std::vector<Delaunay3::Vertex_handle> vecVertexHandles;
	__gnu_cxx ::hash_map<Delaunay3::Vertex_handle, int, HashVertHandle, EqVertHandle> hmapVertexHandleToIndex;
	int numKept = 0, numNotKept = 0;
...
//				tmpTri = PointD3(hmapVertexHandleToIndex[vecTri[0]], hmapVertexHandleToIndex[vecTri[1]], hmapVertexHandleToIndex[vecTri[2]]);
				...
				// SPEARATE TRIANGLES => create new points for each triangle
				points.push_back(vecTri[0]->point());
				points.push_back(vecTri[1]->point());
				points.push_back(vecTri[2]->point());

				tmpTri =  PointD3(nextPointId++, nextPointId++, nextPointId++);
...
void OutputCreator::writeSeparateTrianglesToOFF(const std::string filename, int lastCam) {

	std::ofstream outfile;

	std::vector<PointD3> points;
	std::vector<PointD3> tris;
	tetrahedraToSeparateTriangles(points, tris, lastCam);
