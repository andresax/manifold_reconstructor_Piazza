...
void ManifoldMeshReconstructor::insertNewPointsFromCam(int camIdx, bool incremental) {
	...
	logger_.endEventAndPrint("Init Steiner Grid \t\t\t", true);
	statInitSteinerGrid_ = logger_.getLastDelta();
	...
	statShrink_ = logger_.getLastDelta();
	...
	logger_.endEventAndPrint("Insert Vertices \t\t\t", true);
	statInsertVertices_ = logger_.getLastDelta();
	...
	logger_.endEventAndPrint("Ray Tracing 1 \t\t\t\t", true);
	statRayTracing1_ = logger_.getLastDelta();
...
void ManifoldMeshReconstructor::rayTracingFromCam(int idxCam) {
	...
	logger_.endEventAndPrint("Ray Tracing 2 \t\t\t\t", true);
	statRayTracing2_ = logger_.getLastDelta();
...
void ManifoldMeshReconstructor::saveManifoldSeparateTriangles(const std::string filename) {
	outputM_->writeSeparateTrianglesToOFF(filename);
}
...
bool ManifoldMeshReconstructor::cellTraversalExitTest(
		int & f, int & fold, const Delaunay3::Cell_handle& tetCur, const Delaunay3::Cell_handle &tetPrev, const Segment & constraint) {

	// Let f be the entry face's index.
	for (int curNeig = 0; curNeig < 4; ++curNeig) {
		if (tetCur->neighbor(curNeig) == tetPrev) fold = curNeig;
	}
