...
// Public Methods
	...
	void addIntersection(int camIndex, int featureIndex, float vote) {
		m_setIntersections.insert(m_setIntersections.end(), FSConstraint(camIndex, featureIndex, vote));
	}

	void addIntersection(int camIndex, int featureIndex, float vote, int vertexidx) {
		m_setIntersections.insert(m_setIntersections.end(), FSConstraint(camIndex, featureIndex, vertexidx, vote));
	}

	void removeIntersection(int camIndex, int featureIndex) {
...
//			// Quick rT>
	...
	//	}eturn / rejection on the case that m_nMaxConstraintsKept == 1.
//			if (HEURISTIC_K == 1)
//			return;
//
